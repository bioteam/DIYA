...
set awkpath = /site/glimmer3/scripts
set glimmerpath = /arch/bin

# add/change glimmer options here
set glimmeropts = "-o0 -g110 -t99 -l"
...
step1:
# concatenate all reads with a stop-codon spacer in between
set TMPF = ${tag}-tmp.fa
echo '>longorf-tmp' > $TMPF
cat $genome | grep -v '>' | sed 's/$/TAATAGTGATCACTATTA/' >> $TMPF
# Find long, non-overlapping orfs to use as a training set
echo "Step 1 of ${numsteps}:  Finding long orfs for training"
# was "-t 1.15", was "-g 90"
set longorfscmd = "$glimmerpath/long-orfs -g 30 $TMPF $tag.longorfs"
echo "command is $longorfscmd"
$longorfscmd
if  ($status != 0)  then
...
$glimmerpath/extract -t $TMPF $tag.longorfs > $tag.train
if  ($status != 0)  then
